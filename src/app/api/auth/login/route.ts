...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (isNativeForm) return NextResponse.redirect(new URL('/login?error=invalid', req.url));
      return NextResponse.json({ error: 'Invalid request body' }, { status: 400 });
    }
    if (!email || !password) {
      if (isNativeForm) return NextResponse.redirect(new URL('/login?error=missing', req.url));
    ...
    if (!user || !passwordOk) {
      if (isNativeForm) return NextResponse.redirect(new URL('/login?error=credentials', req.url));
    ...
    if (!token) {
      if (isNativeForm) return NextResponse.redirect(new URL('/login?error=server', req.url));
    ...
    if (isNativeForm) {
      ...
      redirect.cookies.set(AUTH_COOKIE_NAME, token, cookieOpts);
      return redirect;
    }
