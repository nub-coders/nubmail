...
export async function initEmailAccounts(domain: string) {
  if (!domain) {
    console.log('[init-email-accounts] No DOMAIN provided, skipping email account creation');
    return;
  }
    ...
    if (existingAccount.rows.length === 0) {
      await pgQuery(
        `INSERT INTO email_accounts (
          email_address, use_built_in_smtp, smtp_host, smtp_port, smtp_user, smtp_pass
        ) VALUES ($1, $2, $3, $4, $5, $6)`,
        [emailAddress, true, null, null, null, null]
      );
